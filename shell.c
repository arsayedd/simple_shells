...
_state(0);
if (argc == 2)
check_files_type(argv[1], &fd, &active_mode);
_enviornment(dup_2D(envp), 1);
shell_cores(symbol, fd, active_mode);
_enviornment(NULL, 0);
_alias(NULL, 0);
return (State);
}
...
* Authors: Ahmed Raafat & Ahmed Abdelanasser
*/
void check_files_type(char *filename, int *fd, int *active_mode)
{
...
* Authors: Ahmed Raafat & Ahmed Abdelanasser
*/
void shell_cores(char *symbol, int fd, int active_mode)
{
...
get_command(str, &c_command, &command_size);
}
if (read_size == 0)
{
_Free(str);
