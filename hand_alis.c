...
/**
* hand_alias - hand alias command (char **)
...
alias_lisr[i][_strllen(key)] == '=')
{
char *str = _strddup(alias_lisr[i]);
...
/**
* gt_alias_value - gt value of alias
...
{
path = _strddup(key);
path = _realloc(path, _strllen(path) + 2);
_strcat(path, "=");
...
{
value = _strddup(cur_env + _strllen(path));
_Free(path);
return (value);
}
i++;
