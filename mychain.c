...
/**
 * is_new_chain - test if current char in buffer is a chain delimeter
...
/**
 * check_new_chain - checks we should continue chaining based on last status
 * @info: the parameter struct
 * @buf: the char buffer
 * @p: address of current position in buf
...
/**
 * replace_new_alias - replaces an aliases in the tokenized string
 * @info: the parameter struct
 *
...
/**
 * replace_new_vars - replaces vars in the tokenized string
 * @info: the parameter struct
 *
 * Return: 1 if replaced, 0 otherwise
 */
int replace_new_vars(new_info_t *info)
{
    ...
    for (i = 0; info->argv[i]; i++)
    {
        ...
        {
            replace_new_string(&(info->argv[i]),
                               new_strdup(convert_number(info->status, 10, 0)));
            continue;
        }
        if (!new_strcmp(info->argv[i], "$$"))
        {
            replace_new_string(&(info->argv[i]),
                               new_strdup(convert_number(getpid(), 10, 0)));
            continue;
        }
        ...
        {
            replace_new_string(&(info->argv[i]),
                               new_strdup(new_strchr(node->str, '=') + 1));
            continue;
        }
...
/**
 * replace_new_string - replaces string
